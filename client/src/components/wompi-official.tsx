...
export function WompiOfficial({
  ...
  // Create the official Wompi Widget HTML when config is ready
  useEffect(() => {
    if (!widgetConfig || !widgetContainerRef.current) return;
    ...
    // Create the official Wompi form with script tag as per documentation
    ...
    script.setAttribute('data-currency', widgetConfig.currency);
    script.setAttribute('data-amount-in-cents', widgetConfig.amountInCents.toString());
    script.setAttribute('data-reference', widgetConfig.reference);
    script.setAttribute('data-signature:integrity', widgetConfig.signature);
    
    // Optional parameters
    if (widgetConfig.redirectUrl) {
      script.setAttribute('data-redirect-url', widgetConfig.redirectUrl);
    }
    
    // Customer data
    if (widgetConfig.customerData) {
      if (widgetConfig.customerData.email) {
        script.setAttribute('data-customer-data:email', widgetConfig.customerData.email);
      }
      if (widgetConfig.customerData.full_name) {
        script.setAttribute('data-customer-data:full-name', widgetConfig.customerData.full_name);
      }
      if (widgetConfig.customerData.phone_number) {
        script.setAttribute('data-customer-data:phone-number', widgetConfig.customerData.phone_number);
      }
    }
    
    // Shipping address
    if (widgetConfig.shippingAddress) {
      if (widgetConfig.shippingAddress.address_line_1) {
        script.setAttribute('data-shipping-address:address-line-1', widgetConfig.shippingAddress.address_line_1);
      }
      if (widgetConfig.shippingAddress.city) {
        script.setAttribute('data-shipping-address:city', widgetConfig.shippingAddress.city);
      }
      if (widgetConfig.shippingAddress.region) {
        script.setAttribute('data-shipping-address:region', widgetConfig.shippingAddress.region);
      }
      if (widgetConfig.shippingAddress.country) {
        script.setAttribute('data-shipping-address:country', widgetConfig.shippingAddress.country);
      }
      if (widgetConfig.shippingAddress.phone_number) {
        script.setAttribute('data-shipping-address:phone-number', widgetConfig.shippingAddress.phone_number);
      }
    }
    ...
    console.log('Wompi Widget rendered with config:', widgetConfig);
  ...
  return (
    <Card className="w-full max-w-md mx-auto bg-black border-gray-600">
      <CardHeader>
        ...
        <div className="text-center">
          ...
          <div className="text-xs text-gray-500 mt-1">
            🔒 Integración oficial según documentación Wompi
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {/* Widget Container - Official Wompi Widget will be rendered here */}
        <div className="space-y-3">
          <div className="text-center text-sm text-gray-300 mb-3">
            🛡️ Widget Oficial de Wompi:
          </div>
        ...
        <div className="text-center text-xs text-gray-400">
          ...
          <br />
          {widgetConfig ? '✅ Widget oficial cargado' : '⏳ Preparando widget...'}
        </div>
      </CardContent>
    </Card>
  );
